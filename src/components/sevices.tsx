...
function Sevices() {
  ...
  function creatediv(text: string, key: number) {
    return (
      <div className="inline-flex " key={key}>
        <div className="text-xl text-slate-600 font-light hover:text-slate-900 mr-5 hover:font-normal text-left mb-8 hover:underline decoration-[#88A855] w-auto">
          {" "}
          {text}{" "}
        </div>

        <svg
          className="text-[#88A855]"
          width="30"
          height="30"
          viewBox="0 0 15 15"
          fill="none"
    ...
    <>
      ...
      <div className="p-9 inline-flex bg-white w-full" id="services">
        <div className="mx-12 w-1/2 pr-8">
          ...
          <div className="mt-5">
            {techStacksweb.map((techStack, index) => (
              <span className="font-light" key={index}>
                {techStack}
                <span className="text-green-500"> / </span> <span></span>
              </span>
            ))}{" "}
          ...
          <div className="mt-5">
            {techStacksmobile.map((techstack, index) => (
              <span className="font-light" key={index}>
                {techstack} <span className="text-green-500"> / </span>
              </span>
            ))}{" "}
            <span className="font-light">Python</span>
          </div>
        ...
        <div>
          <div className="flex flex-col ">{devtypes.map(creatediv)}</div>
        </div>
      </div>
    </>
  );
